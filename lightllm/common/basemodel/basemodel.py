...
from lightllm.common.basemodel.triton_kernel.splitfuse_copy_kv_index_to_req import (
...
class TpPartBaseModel:
    ...
    def _init_req_manager(self):
        self.req_manager = ReqManager(
            self.max_req_num, self.max_seq_length, self.mem_manager
        )
        return
    ...
    def _prefill(
        ...
        if alloc_mem is not None:
            ...
            infer_state.key_buffer = torch.empty(
                (infer_state.total_token_num, self.tp_k_head_num_, self.head_dim_),
                dtype=torch.float16,
                device="cuda",
            )
            infer_state.value_buffer = torch.empty(
                (infer_state.total_token_num, self.tp_v_head_num_, self.head_dim_),
                dtype=torch.float16,
                device="cuda",
            )
    ...
    def splitfuse_forward(
        ...
        if alloc_mem is not None:
            ...
            infer_state.mem_index = alloc_mem
            infer_state.key_buffer = torch.empty(
                (alloc_size, self.tp_k_head_num_, self.head_dim_),
                dtype=torch.float16,
                device="cuda",
            )
            infer_state.value_buffer = torch.empty(
                (alloc_size, self.tp_v_head_num_, self.head_dim_),
                dtype=torch.float16,
                device="cuda",
            )
    ...
    def _splitfuse_forward(self, input_ids, infer_state: SplitFuseInferStateInfo):
        cuda_input_ids = input_ids
        input_embs = self.pre_infer.splitfuse_forward(
            cuda_input_ids, infer_state, self.pre_post_weight
        )
        for i in range(self.layers_num):
            input_embs = self.layers_infer[i].splitfuse_forward(
                input_embs, infer_state, self.trans_layers_weight[i]
            )

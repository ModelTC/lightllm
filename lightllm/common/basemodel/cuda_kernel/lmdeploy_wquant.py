...
@torch.no_grad()
def quantize_int4_lmdeploy(
    weight, group_size=128, tp_rank=0, pack_order=[0, 2, 4, 6, 1, 3, 5, 7]
):
    """
    weight: [K, N]
    return:
    ...
    logger.debug(
        "tp_rank: {} quantize_int4_lmdeploy for K={} N={} ...".format(tp_rank, K, N)
    )
    ...
    # (N, K)
    weight = (
        (weight / scale + zero_point).round().clamp(0, 15).to(torch.int32).view(N, K)
    )
    # (N, K//group_size)
    ...
    qzeros = torch.zeros(
        (N // 8, K // group_size), dtype=torch.int32, device=weight.device
    )
    ...
    convert_s4_k_m8(
        qweight_new, scale_zeros, temp, qweight, scale, qzeros, N, K, group_size
    )
    temp = None
    scale = None

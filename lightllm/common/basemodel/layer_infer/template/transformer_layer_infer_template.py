...
class TransformerLayerInferTpl(TransformerLayerInfer):
    ...
    def _att_norm(self, input, infer_state: InferStateInfo,
                  layer_weight) -> torch.Tensor:
        raise Exception("need to impl")

    def _ffn_norm(self, input, infer_state: InferStateInfo,
                  layer_weight) -> torch.Tensor:
        raise Exception("need to impl")

    def _pre_cache_kv(self, infer_state: InferStateInfo,
    ...
    def _get_qkv(self, input, cache_k, cache_v,
                 infer_state: InferStateInfo, layer_weight) -> torch.Tensor:
        raise Exception("need to impl")

    def _post_cache_kv(self, cache_k, cache_v,
                       infer_state: InferStateInfo, layer_weight):
        mem_manager = infer_state.mem_manager
        if infer_state.is_prefill:
            destindex_copy_kv(cache_k,
                              infer_state.prefill_mem_index,
                              mem_manager.key_buffer[self.layer_num_])
            destindex_copy_kv(cache_v,
                              infer_state.prefill_mem_index,
    ...
    def _context_attention_kernel(
            self, q, k, v, infer_state: InferStateInfo, layer_weight) -> torch.Tensor:
        raise Exception("need to impl")

    def _token_attention_kernel(
            self, q, infer_state: InferStateInfo, layer_weight) -> torch.Tensor:
        raise Exception("need to impl")

    def _get_o(self, input, infer_state: InferStateInfo,
               layer_weight) -> torch.Tensor:
        raise Exception("need to impl")

    def _ffn(self, input, infer_state: InferStateInfo,
             layer_weight) -> torch.Tensor:
        raise Exception("need to impl")
    ...
    def context_forward(self, input_embdings,
                        infer_state: InferStateInfo, layer_weight):
    ...
    def token_forward(self, input_embdings,
                      infer_state: InferStateInfo, layer_weight):

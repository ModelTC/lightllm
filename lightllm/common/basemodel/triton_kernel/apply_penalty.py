...
def _fwd_kernel_apply_penalty(
    ...
    for block_start_index in range(cur_batch_start_index, cur_batch_end_index, BLOCK_P):
        ...
        cur_logits = tl.load(
            cur_offset, mask=(cur_batch_id_offset < cur_batch_end_index) & (batch_ids < vocab_size), other=0.0
        )
        ...
        tl.store(output_ptr, pre_logits, mask=(cur_batch_id_offset < cur_batch_end_index) & (batch_ids < vocab_size))

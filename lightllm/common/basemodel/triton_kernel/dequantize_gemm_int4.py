...
def matmul4_kernel(
    ...
    b_ptrs = b_ptr + ((offs_k[:,
                              None] // infearure_per_bits) * stride_bk + offs_bn[None,
                                                                                 :] * stride_bn)   # (BLOCK_SIZE_K, BLOCK_SIZE_N)
    scales_ptrs = scales_ptr + offs_bn * stride_scales_n   # (BLOCK_SIZE_N,)
    # zeros_ptrs is set up such that it repeats elements along the N axis 8
    # times
    zeros_ptrs = zeros_ptr + \
        ((offs_bn // infearure_per_bits) * stride_zeros_n)   # (BLOCK_SIZE_N,)
    # shifter is used to extract the 4 bits of each element in the 32-bit word
    # from B and zeros
    shifter = (offs_k % infearure_per_bits) * bits
    zeros_shifter = (offs_bn % infearure_per_bits) * bits
    # If G == 1, scales and zeros are the same for all K, so we can load them
    # once
    if NO_GROUPS:
    ...
    # It's calculating BLOCK_SIZE_M batches in parallel, and for each batch,
    # BLOCK_SIZE_N outfeatures in parallel
...
def matmul_dequantize_int4_gptq(x: torch.FloatTensor, qweight: torch.IntTensor, scales: torch.FloatTensor,
                                qzeros: torch.IntTensor, group_size, output=None) -> torch.FloatTensor:
    """
    ...
    """
    assert x.shape[-1] == (qweight.shape[0] *
                           8), "A must be a multiple of 8 in the last dimension"
...
def matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K * SPLIT_K)):
        ...
        b_shift_bits = (offs_k[:, None] %
                        8) * 4  # assert BLOCK_SIZE_K % 8 == 0
...
@triton.autotune(
    configs=[
        ...
        triton.Config({'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 32},
                      num_stages=4, num_warps=4),
        triton.Config({'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 32},
...
def dequantize_kernel(
    ...
    bzp_offs = (offs_k[:, None] // group_size) * \
        stride_bzpk + (offs_n[None, :] // 8) * stride_bzpn
    bs_offs = (offs_k[:, None] // group_size) * \
    ...
    fp_weight = (((int32_b >> b_shift) & 0xF) -
                 ((zp_b >> bzp_shift) & 0xF)) * scale_b
...
def matmul_dequantize_int4_s1(
        a, b, b_scale, b_zero_point, group_size=128, out=None):
    """
...
def quantize_int4(weight, group_size=128):
    ...
    '''
    weight = None
    return int_weight.transpose(1, 0).contiguous(), scale.transpose(
        1, 0).contiguous(), int_zero_point.transpose(1, 0).contiguous()
...
def benchmark(M, provider):
    ...
    if provider == 'triton-s1':
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul_dequantize_int4_s1(
                a, intb, b_scale, bzp, 64), quantiles=quantiles)

        def perf(ms): return 2 * M * N * K * 1e-12 / (ms * 1e-3)
    if provider == 'triton-s2':
        intb, b_scale, bzp = quantize_int4(b, group_size=64)
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul_dequantize_int4_s2(
                a, intb, b_scale, bzp, 64), quantiles=quantiles)

        def perf(ms): return 2 * M * N * K * 1e-12 / (ms * 1e-3)
    if provider == 'dequantize':
        intb, b_scale, bzp = quantize_int4(b, group_size=64)
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: dequantize_int4(
            intb, b_scale, bzp, 'cuda', torch.float16, 64), quantiles=quantiles)

        def perf(ms): return 2 * M * K * 1e-9 / (ms * 1e-3)
    if provider == 'triton-gptq':
        intb, b_scale, bzp = quantize_int4(b, group_size=64)
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul_dequantize_int4_gptq(
                a, intb, b_scale, bzp, 64), quantiles=quantiles)

...
def _fwd_kernel_copy_kv_index_to_req(
    ...
    for block_start in range(0, q_split_len, BLOCK_M):
        read_index = tl.load(
            memindex + q_mem_start + block_start + off_m,
            mask=q_mem_start + block_start + off_m < q_mem_end,
            other=0,
        )
        tl.store(
            req_to_token_indexs
            + cur_req_idx * stride_req_to_token_b
            + (block_start + store_start + off_m),
            read_index,
...
@torch.no_grad()
def splitfuse_copy_kv_index_to_req(
    req_to_token_indexs, b_req_idx, b_split_seq_len, b_seq_len, memindex
):

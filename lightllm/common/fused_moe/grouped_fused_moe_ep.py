...
def fused_experts_impl(
    ...
    _buffer.get_dispatch_layout(topk_idx, num_experts, previous_event=previous_event, async_finish=False,
                                    allocate_on_comm_stream=False)

    if prefill:
        # normal dispatch
        recv_x, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, handle, event = \
        _buffer.dispatch((qinput_tensor, input_scale), topk_idx=topk_idx, topk_weights=topk_weights,
        ...
        # normal combine
        combined_x, _, event = _buffer.combine(gemm_out_b, handle, async_finish=False, previous_event=previous_event,
                                           allocate_on_comm_stream=False)
...
def init_dist(local_rank: int, num_local_ranks: int):
    ...
    ip = os.getenv('MASTER_ADDR', '10.120.114.74')

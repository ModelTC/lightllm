...
class MemoryManager:
    def __init__(self, size, dtype, head_num, head_dim,
                 layer_num, always_copy=False):
        self.mem_state = torch.ones((size,), dtype=torch.bool, device="cuda")
        self._mem_cum_sum = torch.empty(
            (size,), dtype=torch.int32, device="cuda")
        ...
        self.key_buffer = [
            torch.empty(
                (size,
                 ...
                 head_dim),
                dtype=dtype,
                device="cuda") for _ in range(layer_num)]
        self.value_buffer = [
            torch.empty(
                (size,
                 head_num,
    ...
    def alloc_contiguous(self, need_size):
        ...
        loc_sums = self._mem_cum_sum[need_size - 1:] - self._mem_cum_sum[0:sum_size -
                                                                         need_size + 1] + self.mem_state[0:sum_size - need_size + 1]
        can_used_loc = self.indexes[0:sum_size -
                                    need_size + 1][loc_sums == need_size]

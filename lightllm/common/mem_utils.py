...
from lightllm.common.ppl_int2kv_mem_manager import PPLINT2KVMemoryManager
...
def select_mem_manager_class(mode):
    ...
    if "ppl_int8kv" in mode or "ppl_int8kv_flashdecoding" in mode:
        ...
        logger.info(f"Model kv cache using mode {mode}")
    elif "ppl_int2kv_flashdecoding" in mode:
        memory_manager_class = PPLINT2KVMemoryManager
        logger.info(f"Model kv cache using mode {mode}")

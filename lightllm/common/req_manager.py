...
class ReqManager:
    ...
    def add_req(self, req_len, token_index):
        req_index = self.alloc(1)[0]
        self.req_to_token_indexs[req_index][:req_len] = token_index
        return req_index

    def add_reqs(self, req_lens, token_indices):
        req_ids = []
        cnt = 0
        assert sum(req_lens) == len(token_indices), f"ERROR !!! {sum(req_lens)} != {len(token_indices)}"
        for rlen in req_lens:
            tid = token_indices[cnt : cnt + rlen]
            cnt += rlen
            rid = self.add_req(rlen, tid).item()
            req_ids.append(rid)
        return req_ids
    ...
    def free_reqs(self, free_req_indexs):
        self.can_use_req_size += len(free_req_indexs)
        self.req_state[free_req_indexs] = 0

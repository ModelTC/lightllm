# Adapted from
# https://github.com/vllm-project/vllm/blob/v0.6.4.post1/vllm/_custom_ops.py
...
# for vllm_quant.py
torch.ops._C.cutlass_scaled_mm = torch.ops.vllm_total.cutlass_scaled_mm
...
def silu_and_mul(out: torch.Tensor, x: torch.Tensor) -> None:
    torch.ops.vllm_total.silu_and_mul(out, x)


def gelu_and_mul(out: torch.Tensor, x: torch.Tensor) -> None:
    torch.ops.vllm_total.gelu_and_mul(out, x)


def gelu_tanh_and_mul(out: torch.Tensor, x: torch.Tensor) -> None:
    torch.ops.vllm_total.gelu_tanh_and_mul(out, x)


def fatrelu_and_mul(out: torch.Tensor, x: torch.Tensor, threshold: float = 0.0) -> None:
    torch.ops.vllm_total.fatrelu_and_mul(out, x, threshold)


def gelu_fast(out: torch.Tensor, x: torch.Tensor) -> None:
    torch.ops.vllm_total.gelu_fast(out, x)


def gelu_new(out: torch.Tensor, x: torch.Tensor) -> None:
    torch.ops.vllm_total.gelu_new(out, x)


def gelu_quick(out: torch.Tensor, x: torch.Tensor) -> None:
    torch.ops.vllm_total.gelu_quick(out, x)
...
def paged_attention_v1(
    ...
    torch.ops.vllm_total.paged_attention_v1(
        out,
        query,
        key_cache,
        value_cache,
...
def paged_attention_v2(
    ...
    torch.ops.vllm_total.paged_attention_v2(
        out,
        exp_sum,
        max_logits,
        tmp_out,
...
def rotary_embedding(
    ...
    torch.ops.vllm_total.rotary_embedding(positions, query, key, head_size, cos_sin_cache, is_neox)
...
def batched_rotary_embedding(
    ...
    torch.ops.vllm_total.batched_rotary_embedding(
...
def rms_norm(out: torch.Tensor, input: torch.Tensor, weight: torch.Tensor, epsilon: float) -> None:
    torch.ops.vllm_total.rms_norm(out, input, weight, epsilon)


def fused_add_rms_norm(input: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor, epsilon: float) -> None:
    torch.ops.vllm_total.fused_add_rms_norm(input, residual, weight, epsilon)
...
def advance_step_flashattn(
    ...
    return torch.ops.vllm_total.advance_step_flashattn(
        num_seqs,
        num_queries,
        block_size,
        input_tokens,
...
def advance_step_flashinfer(
    ...
    return torch.ops.vllm_total.advance_step_flashinfer(
        num_seqs,
        num_queries,
        block_size,
        input_tokens,
...
#     return torch.ops.vllm_total.awq_dequantize(qweight, scales, zeros, split_k_iters,
#                                        thx, thy)
...
#     return torch.ops.vllm_total.awq_gemm(input, qweight, qzeros, scales, split_k_iters)
...
def gptq_gemm(
    ...
    return torch.ops.vllm_total.gptq_gemm(a, b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx, use_exllama, bit)


if hasattr(torch.ops.vllm_total, "gptq_gemm"):

    @register_fake("vllm_total::gptq_gemm")
    def _gptq_gemm_fake(
        a: torch.Tensor,
        b_q_weight: torch.Tensor,
        b_gptq_qzeros: torch.Tensor,
...
def gptq_shuffle(q_weight: torch.Tensor, q_perm: torch.Tensor, bit: int) -> None:
    torch.ops.vllm_total.gptq_shuffle(q_weight, q_perm, bit)
...
def marlin_gemm(
    ...
    return torch.ops.vllm_total.marlin_gemm(a, b_q_weight, b_scales, workspace, size_m, size_n, size_k)
...
def gptq_marlin_24_gemm(
    ...
    return torch.ops.vllm_total.gptq_marlin_24_gemm(
        a, b_q_weight, b_meta, b_scales, workspace, b_q_type.id, size_m, size_n, size_k
    )


if hasattr(torch.ops.vllm_total, "gptq_marlin_24_gemm"):

    @register_fake("vllm_total::gptq_marlin_24_gemm")
    ...
    @register_fake("vllm_total::gptq_marlin_gemm")
    ...
    @register_fake("vllm_total::ggml_dequantize")
    def _ggml_dequantize_fake(W: torch.Tensor, quant_type: int, m: torch.SymInt, n: torch.SymInt) -> torch.Tensor:
        return torch.empty((m, n), dtype=torch.float16, device=W.device)

    @register_fake("vllm_total::ggml_mul_mat_vec_a8")
    ...
    @register_fake("vllm_total::ggml_mul_mat_a8")
    ...
    @register_fake("vllm_total::marlin_qqq_gemm")
    ...
    @register_fake("vllm_total::marlin_gemm")
    ...
    @register_fake("vllm_total::awq_dequantize")
    ...
    @register_fake("vllm_total::awq_gemm")
    def _awq_gemm_fake(
        input: torch.Tensor,
        qweight: torch.Tensor,
        qzeros: torch.Tensor,
        ...
        return torch.empty(
            (split_k_iters, num_in_feats, qweight.size(1) * 8), dtype=input.dtype, device=input.device
        ).sum(0)

    @register_fake("vllm_total::aqlm_gemm")
    def _aqlm_gemm_fake(
        input: torch.Tensor,
        codes: torch.Tensor,
    ...
    @register_fake("vllm_total::aqlm_dequant")
    ...
    @register_fake("vllm_total::fp8_marlin_gemm")
    ...
    @register_fake("vllm_total::machete_gemm")
    ...
    @register_fake("vllm_total::machete_prepack_B")
    def machete_prepack_B_fake(b_q_weight: torch.Tensor, b_type) -> torch.Tensor:
        return torch.empty_like(b_q_weight, memory_format=torch.contiguous_format)


# cutlass
def cutlass_scaled_mm_supports_fp8(cuda_device_capability: int) -> bool:
    return torch.ops.vllm_total.cutlass_scaled_mm_supports_fp8(cuda_device_capability)
...
def cutlass_scaled_mm(
    ...
    torch.ops.vllm_total.cutlass_scaled_mm(out, a, b, scale_a, scale_b, bias)
...
def cutlass_scaled_mm_azp(
    ...
    torch.ops.vllm_total.cutlass_scaled_mm_azp(out, a, b, scale_a, scale_b, azp_adj, azp, bias)
    return out
...
def aqlm_gemm(
    ...
    return torch.ops.vllm_total.aqlm_gemm(input, codes, codebooks, scales, codebook_partition_sizes, bias)


def aqlm_dequant(codes: torch.Tensor, codebooks: torch.Tensor, codebook_partition_sizes: List[int]) -> torch.Tensor:
    return torch.ops.vllm_total.aqlm_dequant(codes, codebooks, codebook_partition_sizes)
...
def gptq_marlin_repack(
    ...
    return torch.ops.vllm_total.gptq_marlin_repack(b_q_weight, perm, size_k, size_n, num_bits)


# gptq_marlin
def awq_marlin_repack(b_q_weight: torch.Tensor, size_k: int, size_n: int, num_bits: int) -> torch.Tensor:
    return torch.ops.vllm_total.awq_marlin_repack(b_q_weight, size_k, size_n, num_bits)
...
def gptq_marlin_moe_repack(
    ...
    assert size_k % 16 == 0
    output = torch.empty(
        (num_experts, size_k // 16, size_n * (num_bits // 2)), device=b_q_weight.device, dtype=b_q_weight.dtype
    )
    for e in range(num_experts):
        output[e] = torch.ops.vllm_total.gptq_marlin_repack(b_q_weight[e], perm[e], size_k, size_n, num_bits)
    return output
...
def awq_marlin_moe_repack(
    ...
    for e in range(num_experts):
        output[e] = torch.ops.vllm_total.awq_marlin_repack(b_q_weight[e], size_k, size_n, num_bits)
    return output
...
def gptq_marlin_gemm(
    ...
    return torch.ops.vllm_total.gptq_marlin_gemm(
        a,
        b_q_weight,
        b_scales,
        b_zeros,
...
def fp8_marlin_gemm(
    ...
    return torch.ops.vllm_total.fp8_marlin_gemm(a, b_q_weight, b_scales, workspace, num_bits, size_m, size_n, size_k)


# machete
def machete_supported_schedules(b_type) -> List[str]:
    return torch.ops.vllm_total.machete_supported_schedules(b_type.id)
...
def machete_gemm(
    ...
    return torch.ops.vllm_total.machete_gemm(
        a, b_q, b_type.id, b_scales, b_zeros, b_group_size, c, alpha, beta, schedule
    )


def machete_prepack_B(b_q_weight: torch.Tensor, b_type) -> torch.Tensor:
    return torch.ops.vllm_total.machete_prepack_B(b_q_weight, b_type.id)


if hasattr(torch.ops.vllm_total, "permute_cols"):

    @register_fake("vllm_total::permute_cols")
    def _permute_cols_fake(a: torch.Tensor, perm: torch.Tensor) -> torch.Tensor:
        return torch.empty_like(a)


def permute_cols(a: torch.Tensor, perm: torch.Tensor) -> torch.Tensor:
    return torch.ops.vllm_total.permute_cols(a, perm)
...
def scaled_fp8_quant(
    ...
    if scale is None:
        if use_per_token_if_dynamic:
            ...
            torch.ops.vllm_total.dynamic_per_token_scaled_fp8_quant(output, input, scale, scale_ub)
        else:
            scale = torch.zeros(1, device=input.device, dtype=torch.float32)
            torch.ops.vllm_total.dynamic_scaled_fp8_quant(output, input, scale)
    else:
        # num_token_padding not implemented for this case
        assert scale.numel() == 1 or num_token_padding is None
        torch.ops.vllm_total.static_scaled_fp8_quant(output, input, scale)
...
def scaled_int8_quant(
    ...
    if scale is not None:
        ...
        torch.ops.vllm_total.static_scaled_int8_quant(output, input, scale, azp)
    ...
    torch.ops.vllm_total.dynamic_scaled_int8_quant(output, input, input_scales, input_azp)
...
def marlin_qqq_gemm(
    ...
    return torch.ops.vllm_total.marlin_qqq_gemm(a, b_q_weight, s_tok, s_ch, s_group, workspace, size_m, size_n, size_k)


# gguf
def ggml_dequantize(W: torch.Tensor, quant_type: int, m: int, n: int) -> torch.Tensor:
    return torch.ops.vllm_total.ggml_dequantize(W, quant_type, m, n)
...
def ggml_mul_mat_vec_a8(
    ...
    return torch.ops.vllm_total.ggml_mul_mat_vec_a8(W, X, quant_type, row)
...
def ggml_mul_mat_a8(
    ...
    return torch.ops.vllm_total.ggml_mul_mat_a8(W, X, quant_type, row)
...
def causal_conv1d_fwd(
    ...
    torch.ops.vllm_total.causal_conv1d_fwd(
        x, weight, bias_, conv_states, query_start_loc, cache_indices, has_initial_state, silu_activation, pad_slot_id
    )
...
def causal_conv1d_update(
    ...
    torch.ops.vllm_total.causal_conv1d_update(
        x, conv_state, weight, bias_, silu_activation, cache_seqlens, conv_state_indices, pad_slot_id
    )
...
def selective_scan_fwd(
    ...
    torch.ops.vllm_total.selective_scan_fwd(
        u,
        delta,
        A,
        B,
...
for k, v in names_and_values.items():
    ...
    if (
        ...
        and any(arg is torch.Tensor or arg == "torch.Tensor" for arg in v.__annotations__.values())
    ):

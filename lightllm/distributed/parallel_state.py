# Adapted from
# https://github.com/vllm-project/vllm/blob/v0.6.3.post1/vllm/distributed/parallel_state.py
...
class GroupCoordinator:
    ...
    def graph_capture(self, graph_capture_context: Optional[GraphCaptureContext] = None):
        ...
        with torch.cuda.stream(stream), maybe_ca_context:
            ...
            if not pynccl_comm:
                ...
                maybe_pynccl_context = pynccl_comm.change_state(enable=True, stream=torch.cuda.current_stream())
    ...
    def _all_reduce_in_place(self, input_: torch.Tensor) -> None:
        ...
        #     ipex.distributed.all_reduce(input_, group=self.device_group)
            ...
            original_all_reduce(input_, group=self.device_group)
...
def graph_capture():
    ...
    """
    with get_tp_group().graph_capture() as context, get_pp_group().graph_capture(context):
        yield context
...
def init_distributed_environment(
    ...
    if not torch.distributed.is_initialized():
        ...
        torch.distributed.init_process_group(
            backend=backend, init_method=distributed_init_method, world_size=world_size, rank=rank, timeout=timeout
        )
    # set the local rank
...
def initialize_model_parallel(
    ...
    _PP = init_model_parallel_group(
        group_ranks, get_world_group().local_rank, backend, use_custom_allreduce=False, group_name="pp"
    )
...
def in_the_same_node_as(pg: ProcessGroup, source_rank: int = 0) -> List[bool]:
    ...
    with contextlib.suppress(OSError):
        if rank == source_rank and shm:
            shm.unlink()
    original_all_reduce(is_in_the_same_node, group=pg)

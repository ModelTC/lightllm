...
if triton.__version__ >= "2.1.0":
    ...
    def _fwd_kernel(
        ...
        for start_n in range(0, block_mask * (start_m + 1) * BLOCK_M, BLOCK_N):
            ...
            qk += (alibi_m * (start_n + offs_n[None, :])).to(q.dtype)
...
def torch_att(xq, xk, xv, bs, seqlen, num_head, head_dim):
    ...
    attention_mask = attention_mask[:, :seqlen, :seqlen].to(dtype=torch.float16).to('cuda')
...
def test():
    ...
    alibi = torch.Tensor(_get_interleave(H,)).to('cuda')
...
def get_alibi(n_head=40, max_pos=4096):
    slopes = torch.Tensor(_get_interleave(n_head))
    position_point = torch.arange(max_pos) - max_pos + 1
    position_point = position_point.unsqueeze(0).unsqueeze(0).expand(n_head, -1, -1)
    diag = torch.diag(position_point[0])
    position_point = position_point - diag.unsqueeze(0).unsqueeze(0).transpose(-1, -2) 
    alibi = slopes.unsqueeze(1).unsqueeze(1) * position_point.to(slopes.device)
    alibi = alibi.view(n_head, 1, max_pos)
    alibi_mask = torch.triu(_fill_with_neg_inf(torch.zeros([max_pos, max_pos])), 1)
    alibi_out = alibi_mask.unsqueeze(0).to(slopes.device) + alibi
    alibi_out = alibi_out.to(dtype=torch.float16, device='cuda')
    return alibi_out

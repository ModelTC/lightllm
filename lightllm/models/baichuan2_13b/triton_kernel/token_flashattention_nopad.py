...
from lightllm.models.bloom.triton_kernel.token_attention_nopad_softmax import token_softmax_fwd
from lightllm.models.bloom.triton_kernel.token_attention_nopad_reduceV import token_att_fwd2
...
def torch_att(xq, xk, xv, bs, seqlen, num_head, head_dim):
    ...
    from context_flashattention_nopad import get_alibi
    attention_mask = get_alibi(n_head=num_head)
    attention_mask = attention_mask[:, seqlen-1:seqlen, :seqlen].to(dtype=torch.float16).to('cuda')
    logics += attention_mask.view(num_head, -1).transpose(1, 0).reshape(1, seqlen, num_head)
    logics = torch.max(logics, torch.tensor(torch.finfo(logics.dtype).min))
...
def test():
    ...
    Z, H, N_CTX, D_HEAD = 4, 40, 2048, 128
    dtype = torch.float16
    ...
    from context_flashattention_nopad import _get_interleave
    alibi = torch.Tensor(_get_interleave(H)).to('cuda') # [40]
    ...
    token_attention_fwd(q, k, v, o, alibi, b_loc, b_start_loc, b_seq_len, max_input_len)
    import time

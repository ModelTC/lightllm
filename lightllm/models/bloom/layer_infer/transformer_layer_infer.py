...
class BloomTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    def _att_norm(self, input, infer_state: InferStateInfo,
                  layer_weight: BloomTransformerLayerWeight) -> torch.Tensor:
    ...
    def _get_qkv(self, input, cache_k, cache_v, infer_state: InferStateInfo,
                 layer_weight: BloomTransformerLayerWeight) -> torch.Tensor:
        q = torch.addmm(layer_weight.q_bias_,
                        input.view(-1,
                                   self.embed_dim_),
    ...
    def _context_attention_kernel(self, q, k, v, infer_state: InferStateInfo,
                                  layer_weight: BloomTransformerLayerWeight) -> torch.Tensor:
    ...
    def _token_attention_kernel(self, q, infer_state: InferStateInfo,
                                layer_weight: BloomTransformerLayerWeight) -> torch.Tensor:
        o_tensor = torch.empty_like(q)
    ...
    def _ffn(self, input, infer_state: InferStateInfo,
             layer_weight: BloomTransformerLayerWeight) -> torch.Tensor:
        ffn1_out = torch.addmm(layer_weight.ffn_1_bias_,
                               input.view(-1,
                                          self.embed_dim_),

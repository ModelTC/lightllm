    ...
    def _load_qkvo_weights(self, weights):
        ...
        if (
            ...
            self.v_weight_ = qkv_weight_[
                :,
                n_embed
                + multi_query_group_num * head_dim : n_embed
                + 2 * multi_query_group_num * head_dim,
            ]
        ...
        if (
            f"transformer.encoder.layers.{self.layer_num_}.self_attention.query_key_value.bias"
            in weights
        ):
            qkv_bias_ = weights[
                f"transformer.encoder.layers.{self.layer_num_}.self_attention.query_key_value.bias"
            ].to(self.data_type_)
            self.q_bias_ = qkv_bias_[:n_embed][
                split_n_embed * self.tp_rank_ : split_n_embed * (self.tp_rank_ + 1)
            ]
            self.q_bias_ = self._cuda(self.q_bias_)
            self.k_bias_ = qkv_bias_[
                n_embed : n_embed + head_dim * multi_query_group_num
            ]

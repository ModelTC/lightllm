...
class ChatGLM2TransformerLayerWeight(LlamaTransformerLayerWeight):
    ...
    def _load_qkvo_weights(self, weights):
        ...
        if f"transformer.encoder.layers.{self.layer_num_}.input_layernorm.weight" in weights:
            self.att_norm_weight_ = weights[f"transformer.encoder.layers.{self.layer_num_}.input_layernorm.weight"].to(
        ...
        if f"transformer.encoder.layers.{self.layer_num_}.self_attention.query_key_value.weight" in weights:
            self.qkv_weight_ = weights[f"transformer.encoder.layers.{self.layer_num_}.self_attention.query_key_value.weight"].transpose(
            ...
            self.k_weight_ = self.qkv_weight_[
                :, n_embed:n_embed + head_dim * multi_query_group_num]
        ...
        if f"transformer.encoder.layers.{self.layer_num_}.self_attention.query_key_value.bias" in weights:
            ...
            self.qkv_bias_ = weights[f"transformer.encoder.layers.{self.layer_num_}.self_attention.query_key_value.bias"].to(
                self.data_type_)
            self.q_bias_ = self.qkv_bias_[:n_embed].cuda(
            )[split_n_embed * self.tp_rank_: split_n_embed * (self.tp_rank_ + 1)]
            self.k_bias_ = self.qkv_bias_[
                n_embed: n_embed + head_dim * multi_query_group_num].cuda()
    ...
    def _load_ffn_weights(self, weights):
        ...
        if f"transformer.encoder.layers.{self.layer_num_}.mlp.dense_h_to_4h.weight" in weights:
            self.ffn_1_weight_ = weights[f"transformer.encoder.layers.{self.layer_num_}.mlp.dense_h_to_4h.weight"].to(
                self.data_type_)
        ...
        if f"transformer.encoder.layers.{self.layer_num_}.mlp.dense_4h_to_h.weight" in weights:
            self.ffn_2_weight_ = weights[f"transformer.encoder.layers.{self.layer_num_}.mlp.dense_4h_to_h.weight"].to(
                self.data_type_)

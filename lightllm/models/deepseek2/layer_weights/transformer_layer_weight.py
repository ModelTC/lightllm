    ...
    def load_hf_weights(self, weights):
        if f"model.layers.{self.layer_num_}.self_attn.kv_b_proj.weight" in weights:
            ...
            if self.enable_cc_method:
                cc_k_b_proj_ = kv_b_proj_.view(self.num_attention_heads, self.qk_nope_head_dim * 2, self.kv_lora_rank)[
                    :, : self.qk_nope_head_dim, :
                ].reshape(-1, self.kv_lora_rank)
                weights[f"model.layers.{self.layer_num_}.self_attn.cc_k_b_proj.weight"] = cc_k_b_proj_
                cc_v_b_proj_ = kv_b_proj_.view(self.num_attention_heads, self.qk_nope_head_dim * 2, self.kv_lora_rank)[
                    :, self.qk_nope_head_dim :, :
                ].reshape(-1, self.kv_lora_rank)
                weights[f"model.layers.{self.layer_num_}.self_attn.cc_v_b_proj.weight"] = cc_v_b_proj_
        ...
        if (
            ...
            if self.enable_cc_method:
                ...
                ] = kv_b_proj_scale_.reshape((128, 2, -1))[:, 0, :]
                weights[
                    f"model.layers.{self.layer_num_}.self_attn.cc_v_b_proj." + self.weight_scale_suffix
                ] = kv_b_proj_scale_.reshape([128, 2, -1])[:, 1, :]

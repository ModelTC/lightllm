...
def _fwd_kernel_with_v(
    ...
    for start_n in range(0, block_mask * block_end_loc, BLOCK_N):
        ...
        # update acc
        off_v = k_loc[:, None] * stride_vbs + cur_k_head * stride_vh + offs_d[None, :] * stride_vd
...
@triton.jit
def _fwd_kernel_no_prompt_cache_with_v(
    Q_nope,
    Q_rope,
    K_nope,
    K_rope,
...
@torch.no_grad()
def context_attention_fwd_no_prompt_cache_with_v(
    q_nope, q_rope, k_nope, k_rope, v, o, b_start_loc, b_seq_len, max_input_len, softmax_scale
):
    q_nope_dim = q_nope.shape[-1]
    q_rope_dim = q_rope.shape[-1]
    assert q_nope_dim == k_nope.shape[-1]
    ...
    num_warps = 4 if q_nope_dim <= 64 else 8
    _fwd_kernel_no_prompt_cache_with_v[grid](
        q_nope,
        q_rope,
        k_nope,
        k_rope,

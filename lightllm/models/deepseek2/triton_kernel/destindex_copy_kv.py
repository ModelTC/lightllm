...
def _fwd_kernel_destindex_copy_kv(
    ...
    kv_nope_ptrs = KV_nope + cur_index * stride_kv_nope_bs + stride_kv_nope_h * offs_h[:, None] + stride_kv_nope_d * offs_d_nope[None, :]
    kv_rope_ptrs = KV_rope + cur_index * stride_kv_rope_bs + stride_kv_rope_h * offs_h[:, None] + stride_kv_rope_d * offs_d_rope[None, :]

    o_nope_ptrs = O_nope + dest_index * stride_o_nope_bs + stride_o_nope_h * offs_h[:, None] + stride_o_nope_d * offs_d_nope[None, :]
    o_rope_ptrs = O_rope + dest_index * stride_o_rope_bs + stride_o_rope_h * offs_h[:, None] + stride_o_rope_d * offs_d_rope[None, :]

...
def _fwd_kernel_flash_decode_stage1(
    Q_nope, Q_rope, KV_nope, KV_rope, sm_scale, Req_to_tokens, B_req_idx, B_Seqlen,
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        kv_loc = tl.load(Req_to_tokens + stride_req_to_tokens_b * cur_batch_req_idx +  offs_n_new, mask=offs_n_new < cur_batch_end_index, other=0)
...
@torch.no_grad()
def flash_decode_stage1(q_nope, q_rope, kv_nope, kv_rope, Req_to_tokens, B_req_idx, B_Seqlen, max_len_in_batch, mid_out, mid_out_logsumexp, block_seq):
    BLOCK_SEQ = block_seq
    BLOCK_N = 16

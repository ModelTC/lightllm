...
class LlamaPostLayerInfer(PostLayerInferTpl):
    ...
    def token_forward(
        ...
        if not infer_state.is_splitfuse:
            if infer_state.is_prefill:
                last_index = (
                    torch.cumsum(infer_state.b_seq_len, dim=0, dtype=torch.long) - 1
                )
            ...
            tmp_ = torch.cat(
                [
                    torch.ones(
                        infer_state.decode_req_num, dtype=torch.int32, device="cuda"
                    ),
    ...
    def splitfuse_forward(
        self,
        input_embdings,
        infer_state: SplitFuseInferStateInfo,
        layer_weight: BaseLayerWeight,
        return_logics=False,
    ):
        return self.token_forward(
            input_embdings, infer_state, layer_weight, return_logics=return_logics
        )

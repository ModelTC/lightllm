...
class LlamaTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    def _att_norm(self, input, infer_state: LlamaInferStateInfo,
                  layer_weight: LlamaTransformerLayerWeight) -> torch.Tensor:
        return rmsnorm_forward(
            input, weight=layer_weight.att_norm_weight_, eps=self.eps_)

    def _ffn_norm(self, input, infer_state: LlamaInferStateInfo,
                  layer_weight: LlamaTransformerLayerWeight) -> torch.Tensor:
        return rmsnorm_forward(
            input, weight=layer_weight.ffn_norm_weight_, eps=self.eps_)
    ...
    def _context_attention_kernel(
            self, q, k, v, infer_state: LlamaInferStateInfo, layer_weight) -> torch.Tensor:
        ...
        context_attention_fwd(q.view(-1, self.tp_q_head_num_, self.head_dim_),
                              ...
                              o_tensor.view(-1, self.tp_q_head_num_,
                                            self.head_dim_),
    ...
    def _token_attention_kernel(
            self, q, infer_state: LlamaInferStateInfo, layer_weight) -> torch.Tensor:
        return self._token_decode_attention_mode(q, infer_state)

    def _get_o(self, input, infer_state: LlamaInferStateInfo,
               layer_weight: LlamaTransformerLayerWeight) -> torch.Tensor:
        o_tensor = torch.mm(input.view(-1,
                                       self.tp_o_head_num_ * self.head_dim_),
                            layer_weight.o_weight_)
        return o_tensor

    def _ffn(self, input, infer_state: LlamaInferStateInfo,
             layer_weight: LlamaTransformerLayerWeight) -> torch.Tensor:
        gate_out = torch.mm(input.view(-1, self.embed_dim_),
                            layer_weight.gate_proj)
        torch.nn.functional.silu(gate_out, inplace=True)
        up_out = torch.mm(input.view(-1, self.embed_dim_),
    ...
    def _copy_kv_to_mem_cache(
            self, key_buffer, value_buffer, mem_index, mem_manager):
        if "int8kv" in self.mode:
            ...
            destindex_copy_kv(key_buffer, mem_index,
                              mem_manager.key_buffer[self.layer_num_])
            destindex_copy_kv(value_buffer, mem_index,
                              mem_manager.value_buffer[self.layer_num_])
    ...
    def _token_decode_attention_mode(
            self, q, infer_state: LlamaInferStateInfo):
        if "int8kv" in self.mode:

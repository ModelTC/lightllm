...
def load_ds_weights(data_type, weight_dir, pre_post_layer=None,
                    transformer_layer_list=None, weight_dict=None, prefix="", num_layer=0):
    if weight_dict:
        return weight_dict
    ...
    candidate_files = sorted(
        list(
            filter(
                lambda x: x.endswith('.pt') and x.startswith('layer'),
                files)))
    assert len(
        candidate_files) != 0, "can only support pytorch tensor format for weights."
    if weight_dict:

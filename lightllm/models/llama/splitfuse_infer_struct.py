...
class LlamaSplitFuseInferStateInfo(SplitFuseInferStateInfo):
    ...
    def init_some_extra_state(self, model, input_ids: torch.Tensor):
        ...
        if self.prefill_req_num != 0:
            ...
            position_ids.extend(
                [
                    np.arange(b_start_numpy[i], b_seq_len_numpy[i])
                    for i in range(len(b_seq_len_numpy))
                ]
            )

        position_ids = (
            torch.from_numpy(np.concatenate(position_ids, axis=0)).cuda().view(-1)
        )
        self.position_cos = torch.index_select(model._cos_cached, 0, position_ids).view(
            position_ids.shape[0], -1
        )
        self.position_sin = torch.index_select(model._sin_cached, 0, position_ids).view(
            position_ids.shape[0], -1
        )

        if self.decode_req_num != 0:
            self.other_kv_index = self.req_manager.req_to_token_indexs[
                self.decode_b_req_idx[0], 0
            ].item()
        elif self.prefill_req_num != 0:
            self.other_kv_index = self.req_manager.req_to_token_indexs[
                self.prefill_b_req_idx[0], 0
            ].item()
        return

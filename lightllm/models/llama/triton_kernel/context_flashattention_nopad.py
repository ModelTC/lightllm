...
def _fwd_kernel_int8kv(
    ...
    l_i = tl.zeros([BLOCK_M], dtype=tl.float32)
    acc = tl.zeros([BLOCK_M, BLOCK_DMODEL], dtype=tl.float32)
    # tl.device_print("dasd ", prompt_cache_len)

    # for prompt cache
    for start_n in range(0, prompt_cache_len, BLOCK_N):
        start_n = tl.multiple_of(start_n, BLOCK_N)
        mask = (start_n + offs_n) < prompt_cache_len
        # -- compute qk ----
        kv_loc = tl.load(
            Req_to_tokens + stride_req_to_tokens_b * cur_batch_req_idx + stride_req_to_tokens_s * (start_n + offs_n),
            mask=mask,
        ...
        off_k = kv_loc[None, :] * stride_kbs + cur_kv_head * stride_kh + offs_d[:, None] * stride_kd
        k = tl.load(K + off_k, mask=mask[None, :], other=0.0)
        qk = tl.dot(q, k)
        qk = tl.where(mask[None, :], qk * sm_scale, -1.0e8)
        m_ij = tl.maximum(m_i, tl.max(qk, 1))
        qk -= m_ij[:, None]
        p = tl.math.exp2(qk)
        l_ij = tl.sum(p, 1)
        ...
        # update acc
        off_v = kv_loc[:, None] * stride_vbs + cur_kv_head * stride_vh + offs_d[None, :] * stride_vd
        v = tl.load(V + off_v, mask=mask[:, None], other=0.0)
        p = p.to(v.dtype)
        acc = tl.dot(p, v, acc)
        # update m_i and l_i
        m_i = m_ij
    ...
    for start_n in range(0, block_mask * block_end_loc, BLOCK_N):
        ...
        kv_loc = tl.load(
            ...
            + stride_req_to_tokens_s * (start_n + prompt_cache_len + offs_n),
            mask=(start_n + offs_n) < block_end_loc,
            other=0,
        )
        off_k = kv_loc[None, :] * stride_kbs + cur_kv_head * stride_kh + offs_d[:, None] * stride_kd
...
def context_attention_fwd_int8kv(
    ...
    grid = lambda meta: (triton.cdiv(max_input_len - 2000, meta["BLOCK_M"]), batch * head, 1)
    BLOCK_N = BLOCK_M
    num_warps = 4 if Lk <= 64 else 8
    num_stages = 1
...
def test():
    ...
    Z, H, N_CTX, D_HEAD = 10, 16, 2048, 128
    dtype = torch.float16
    prompt_cache_len = 2000

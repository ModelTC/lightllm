...
if triton.__version__ >= "2.1.0":
    ...
    def _fwd_kernel(
        ...
        for start_n in range(0, block_mask * block_end_loc , BLOCK_N):
            ...
            qk += tl.dot(q, k)
            qk *= sm_scale
            qk = tl.where(offs_m[:, None] + prompt_cache_len >= start_n + offs_n[None, :], qk, -10000000)
...
def test():
    ...
    for i in range(Z):
        ...
        torch_o = torch_att(q[start:end], k[start:end + prompt_cache_len], v[start:end + prompt_cache_len], 1, b_seq_len[i], H, D_HEAD, prompt_cache_len)
        start = end

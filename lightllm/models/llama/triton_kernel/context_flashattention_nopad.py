...
if triton.__version__ >= "2.1.0":
    ...
    def _fwd_kernel(
        ...
        off_k = offs_n[None, :] * stride_kbs + cur_head * \
            stride_kh + offs_d[:, None] * stride_kd
        off_v = offs_n[:, None] * stride_vbs + cur_head * \
...
def torch_att(xq, xk, xv, bs, seqlen, num_head, head_dim):
    ...
    mask = torch.tril(
        torch.ones(
            seqlen,
            seqlen),
        diagonal=0).unsqueeze(0).unsqueeze(0).cuda()
    ...
    output = torch.matmul(scores, values).transpose(
        1, 2).contiguous().reshape(-1, num_head, head_dim)
    return output
...
def test():
    ...
    for i in range(Z):
        ...
        torch_o = torch_att(q[start:end], k[start:end],
                            v[start:end], 1, b_seq_len[i], H, D_HEAD)
        start = end

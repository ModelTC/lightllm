...
def gqa_token_decode_attention_flash_decoding(
    q, infer_state: InferStateInfo, q_head_num, head_dim, cache_k, cache_v, out=None
):
    BLOCK_SEQ = 128
    ...
    if getattr(infer_state, 'mid_o', None) is None:
        ...
        block_batch_ids = torch.from_numpy(
            np.concatenate(
                [
                    np.full(
                        ((b_seq_len_numpy[batch_id] + BLOCK_SEQ - 1) // BLOCK_SEQ,),
                        fill_value=batch_id,
                        dtype=np.int32,
                    )
                    for batch_id in range(len(b_seq_len_numpy))
                ],
                axis=0,
            )
        ).cuda()

        block_start_indexes = torch.from_numpy(
            np.concatenate(
                [
                    np.arange(0, seq_len, BLOCK_SEQ, dtype=np.int32)
                    for seq_len in b_seq_len_numpy
                ],
                axis=0,
            )
        ).cuda()

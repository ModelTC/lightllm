...
def test2():
    ...
    print("max {}".format(torch.max(torch.abs((value_dest * scale_dest).view(B * N_CTX, H, D) - src))))
    print("mean {}".format(torch.mean(torch.abs((value_dest * scale_dest).view(B * N_CTX, H, D) - src))))

...
def _rotary_kernel_gqa(
    ...
    off_dimcos_sin = cur_seq_range[:, None] * stride_cosbs + dim_range0[None, :] * stride_cosd
    cos = tl.load(Cos + off_dimcos_sin, mask=cur_seq_range[:, None] < max_total_len, other=0.0)
    sin = tl.load(Sin + off_dimcos_sin, mask=cur_seq_range[:, None] < max_total_len, other=0.0)

    off_k0 = cur_seq_range[:, None] * stride_kbs + cur_head_index * stride_kh + dim_range0[None, :] * stride_kd
    off_k1 = cur_seq_range[:, None] * stride_kbs + cur_head_index * stride_kh + dim_range1[None, :] * stride_kd

    k0 = tl.load(K + off_k0, mask=(cur_seq_range[:, None] < max_total_len), other=0.0)
    k1 = tl.load(K + off_k1, mask=(cur_seq_range[:, None] < max_total_len), other=0.0)

    out_k0 = k0 * cos - k1 * sin
    out_k1 = k0 * sin + k1 * cos

    tl.store(K + off_k0, out_k0, mask=(cur_seq_range[:, None] < max_total_len))
    tl.store(K + off_k1, out_k1, mask=(cur_seq_range[:, None] < max_total_len))
...
def _rotary_kernel(
    ...
    k0 = tl.load(K + off_k0, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < H), other=0.0)
    k1 = tl.load(K + off_k1, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < H), other=0.0)
    cos = tl.load(Cos + off_dimcos_sin, mask=cur_seq_range[:, None, None] < max_total_len, other=0.0)
    sin = tl.load(Sin + off_dimcos_sin, mask=cur_seq_range[:, None, None] < max_total_len, other=0.0)
    ...
    tl.store(K + off_k0, out_k0, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < H))
    tl.store(K + off_k1, out_k1, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < H))
    return

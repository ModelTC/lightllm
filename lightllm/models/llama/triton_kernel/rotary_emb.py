...
def _rotary_kernel(
    ...
    tl.store(Q + off_q0, out_q0, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < H))
    tl.store(Q + off_q1, out_q1, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < H))
...
def test_rotary_emb(SEQ_LEN, H, D, dtype, eps=1e-5, device='cuda'):
    # create data
    x_shape = (SEQ_LEN, H, D)
    q = -2.3 + 0.5 * torch.randn(x_shape, dtype=dtype, device='cuda')
    k = -1.2 + 0.5 * torch.randn(x_shape, dtype=dtype, device='cuda')
    ...
    # compare
    print("max delta:", torch.max(torch.abs(q_tri - q)))
    print("max delta:", torch.max(torch.abs(k_tri - k)))

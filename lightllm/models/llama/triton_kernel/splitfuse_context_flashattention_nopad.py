...
def _fwd_kernel(
    ...
    off_q = (
        (cur_batch_q_split_start_loc + offs_m[:, None]) * stride_qbs
        + cur_head * stride_qh
        + offs_d[None, :]
    )
    ...
    for start_n in range(
        0, block_mask * (cur_batch_seq_start + (start_m + 1) * BLOCK_M), BLOCK_N
    ):
        ...
        qk *= sm_scale
        qk = tl.where(
            cur_batch_seq_start + offs_m[:, None] >= (start_n + offs_n[None, :]),
            qk,
            float("-inf"),
        )
    ...
    off_o = (
        (cur_batch_q_split_start_loc + offs_m[:, None]) * stride_obs
        + cur_head * stride_oh
        + offs_d[None, :]
    )
    out_ptrs = Out + off_o
...
def splitfuse_context_attention_fwd(
    ...
    grid = (
        prefill_req_num,
        head,
        triton.cdiv(prefill_max_split_seq_len_in_batch, BLOCK),
    )

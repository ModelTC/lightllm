...
def repad_input_embeds(
        input_embeds, infer_state: LlamaMultiModalInferStateInfo):
    ...
    if infer_state.kwargs and 'repad_embeds' in infer_state.kwargs:
        ...
        assert len(repad_embeds) == infer_state.batch_size, "length of repad_embeds != batch_size: {} vs {}!".format(
            len(repad_embeds), infer_state.batch_size)
        ...
        for i, (embeds, offset) in enumerate(repad_embeds):
            ...
            if embeds is None:
                continue
            assert isinstance(
                embeds, torch.Tensor), "given reapd embeds should be torch.Tensor but got {}!".format(
                type(embeds))
            ...
            assert pad_dim == dim, "invalid pad_dim={}, input_embed_dim={}!".format(
                pad_dim, dim)
            assert offset + \
                pad_len <= seq_len, "invalid seq_len={}, offset={}, pad_len={}!".format(
                    seq_len, offset, pad_len)
            input_embeds[start_idx + offset: start_idx +
                         offset + pad_len] = embeds
            print(
                "repad input_embeds start_idx={} offset={} pad_len={}".format(
                    start_idx, offset, pad_len))
    return input_embeds
...
class LlamaMultiModalPreLayerInfer(LlamaPreLayerInfer):
    ...
    def context_forward(self, input_ids, infer_state: LlamaMultiModalInferStateInfo,
    ...
    def token_forward(self, input_ids, infer_state: LlamaMultiModalInferStateInfo,
                      layer_weight: LlamaPreAndPostLayerWeight):

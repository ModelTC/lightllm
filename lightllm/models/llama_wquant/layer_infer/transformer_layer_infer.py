...
from lightllm.common.basemodel.triton_kernel.quantize_gemm_int8 import (
    matmul_quantize_int8,
)
from lightllm.common.basemodel.triton_kernel.dequantize_gemm_int8 import (
...
from lightllm.common.basemodel.cuda_kernel.lmdeploy_wquant import (
    matmul_dequantize_int4_lmdeploy,
)
...
class LlamaTransformerLayerInferWquant(TransformerLayerInferWeightQuantTpl):
    ...
    def _bind_matmul(self):
        if "triton_int8weight" in self.mode:
            func = partial(
                LlamaTransformerLayerInferWquant._wquant_matmul_triton_int8weight_only_quant,
                self,
            )
            ...
            func = partial(
                LlamaTransformerLayerInferWquant._wquant_matmul_triton_int4weight_only_quant,
                self,
            )
            self._wquant_matmul_for_qkv = func
            self._wquant_matmul_for_o = func
            ...
            func = partial(
                LlamaTransformerLayerInferWquant._wquant_matmul_lmdeploy_int4weight_only_quant,
                self,
            )
            self._wquant_matmul_for_qkv = func
            self._wquant_matmul_for_o = func
            ...
            func = partial(
                LlamaTransformerLayerInferWquant._wquant_matmul_ppl_int4weight_only_quant,
                self,
            )
            self._wquant_matmul_for_qkv = func
            self._wquant_matmul_for_o = func
    ...
    def _get_o(
        ...
        o_tensor = self._wquant_matmul_for_o(
            input, quant_weight_params=layer_weight.o_weight_, infer_state=infer_state
        )
        return o_tensor
    ...
    def _wquant_matmul_triton_int4weight_only_quant(
        ...
        if not infer_state.is_splitfuse and infer_state.is_prefill:
            ...
            out = matmul_dequantize_int4_s1(
                input, qweight, scale, zeros, int4_q_group_size, out=out
            )
        else:
            qweight, scale, zeros, int4_q_group_size = quant_weight_params
            out = matmul_dequantize_int4_gptq(
                input, qweight, scale, zeros, int4_q_group_size, output=out
            )
        if bias is None:
            return out
        else:
    ...
    def _wquant_matmul_lmdeploy_int4weight_only_quant(
        ...
        out = matmul_dequantize_int4_lmdeploy(
            input, qweight, scale_zeros, int4_q_group_size
        )
        if bias is None:
            return out
        else:

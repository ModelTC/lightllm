...
class MixtralInferStateInfo(MistralInferStateInfo):
    ...
    def init_some_extra_state(self, model, input_ids: torch.Tensor):
        ...
        self.sliding_window = (4096 if model.config["sliding_window"] is None
                               else model.config["sliding_window"])
        ...
        if self.is_prefill:
            ...
            self.position_cos = torch.index_select(model._cos_cached, 0,
                                                   position_ids).view(
                                                       position_ids.shape[0],
                                                       -1)
            self.position_sin = torch.index_select(model._sin_cached, 0,
                                                   position_ids).view(
                                                       position_ids.shape[0],
                                                       -1)
            position_ids = None
            ...
            for i in range(0, self.batch_size):
                if self.sliding_window < self.b_seq_len[i]:
                    self.b_start_loc_window[
                        i] = self.b_seq_len[i] - self.sliding_window

...
class MixtralTransformerLayerInfer(LlamaTransformerLayerInfer):
    ...
    def _ffn(self, input, infer_state: InferStateInfo,
             layer_weight: MixtralTransformerLayerWeight) -> torch.Tensor:
        router_logits = torch.mm(input.view(-1, self.embed_dim_),
                                 layer_weight.gate)
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        routing_weights, selected_experts = torch.topk(
            routing_weights, infer_state.experts_topk, dim=-1)
        ...
        for expert_idx in range(infer_state.num_local_experts):
            ...
            expert_weights = (routing_weights * expert_mask).sum(dim=-1,
                                                                 keepdim=True)
            w1_out = torch.mm(input.view(-1, self.embed_dim_),
                              expert_layer["w1"])
            torch.nn.functional.silu(w1_out, inplace=True)
            w3_out = torch.mm(input.view(-1, self.embed_dim_),
                              expert_layer["w3"])
            current_hidden_states = w1_out * w3_out
            w1_out, w3_out = None, None
            current_hidden_states = torch.mm(current_hidden_states,
                                             expert_layer["w2"])

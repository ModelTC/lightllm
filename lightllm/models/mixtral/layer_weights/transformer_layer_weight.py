...
class MixtralTransformerLayerWeight(TransformerLayerWeight):
    ...
    def __init__(self,
                 ...
                 network_config,
                 mode=[]):
        super().__init__(layer_num, tp_rank, world_size, data_type,
                         network_config, mode)
        self.experts = [{
            "w1": None,
            "w2": None,
            "w3": None
        } for _ in range(self.network_config_["num_local_experts"])]
        return
    ...
    def verify_load(self):
        ...
        for i in range(len(weights)):
            assert weights[i] is not None, (str(i) + " " +
                                            str(self.layer_num_) + " " +
                                            errors)
        for i in range(self.network_config_["num_local_experts"]):
            assert self.experts[i]["w1"] is not None, ("layer " +
                                                       str(self.layer_num_) +
                                                       " expert " + str(i) +
                                                       " w1 " + errors)
            assert self.experts[i]["w2"] is not None, ("layer " +
                                                       str(self.layer_num_) +
                                                       " expert " + str(i) +
                                                       " w2 " + errors)
            assert self.experts[i]["w3"] is not None, ("layer " +
                                                       str(self.layer_num_) +
                                                       " expert " + str(i) +
                                                       " w3 " + errors)
        return
    ...
    def _load_qkvo_weights(self, weights):
        ...
        if f"model.layers.{self.layer_num_}.self_attn.k_proj.weight" in weights:
            self.k_weight_ = weights[
                f"model.layers.{self.layer_num_}.self_attn.k_proj.weight"]
            self.k_weight_ = self.k_weight_[kv_split_n_embed *
        ...
        if f"model.layers.{self.layer_num_}.self_attn.o_proj.weight" in weights:
            ...
            self.o_weight_ = self.o_weight_[:, q_split_n_embed *
                                            self.tp_rank_:q_split_n_embed *
                                            (self.tp_rank_ + 1), ]
    ...
    def _load_ffn_weights(self, weights):
        if f"model.layers.{self.layer_num_}.post_attention_layernorm.weight" in weights:
            self.ffn_norm_weight_ = self._cuda(weights[
        ...
        for expert_idx in range(self.network_config_["num_local_experts"]):
            if (f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w1.weight"
                    in weights):
                ...
                self.experts[expert_idx]["w1"] = self._cuda(
                    self.experts[expert_idx]["w1"].transpose(0, 1))
            ...
            if (f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w3.weight"
                    in weights):
                self.experts[expert_idx]["w3"] = weights[
                    f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w3.weight"][
                        split_inter_size * self.tp_rank_:split_inter_size *
                        (self.tp_rank_ + 1), :, ]
                self.experts[expert_idx]["w3"] = self._cuda(
                    self.experts[expert_idx]["w3"].transpose(0, 1))
        return

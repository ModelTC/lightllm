    ...
    def _load_ffn_weights(self, weights):
        ...
        for expert_idx in range(self.network_config_["num_local_experts"]):
            if f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w1.weight" in weights:
                self.experts[expert_idx]["w1"] = weights[f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w1.weight"][
            ...
            if f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w2.weight" in weights:
                self.experts[expert_idx]["w2"] = weights[f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w2.weight"][
                    :,
                    split_inter_size * self.tp_rank_ : split_inter_size * (self.tp_rank_ + 1),
                ]
            ...
            if f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w3.weight" in weights:
                self.experts[expert_idx]["w3"] = weights[f"model.layers.{self.layer_num_}.block_sparse_moe.experts.{expert_idx}.w3.weight"][
                    split_inter_size * self.tp_rank_ : split_inter_size * (self.tp_rank_ + 1),
                    :,
                ]

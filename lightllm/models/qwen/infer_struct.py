...
class QwenInferStateInfo(InferStateInfo):
    ...
    def init_some_extra_state(self, model, input_ids : torch.Tensor):
        if self.is_prefill:
            ...
            self.position_sin = []
            self.position_cos = []
            infer_ntk_id = torch.clamp(torch.ceil(torch.log2(self.b_seq_len / model.config.get("seq_length", 2048)) + 1), 0, model.max_ntk_alpha).int()
            for i in range(len(infer_ntk_id)):
                self.position_sin.append(model._sin_cached[infer_ntk_id[i]][position_ids[self.b_start_loc[i]: self.b_start_loc[i] + self.b_seq_len[i]]])
                self.position_cos.append(model._cos_cached[infer_ntk_id[i]][position_ids[self.b_start_loc[i]: self.b_start_loc[i] + self.b_seq_len[i]]])

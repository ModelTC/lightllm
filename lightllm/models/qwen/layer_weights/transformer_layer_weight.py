...
class QwenTransformerLayerWeight(TransformerLayerWeight):
    ...
    def load_hf_weights(self, weights):
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_attn.weight" in weights:
            ...
            q_weights, k_weights, v_weights = torch.split(
                qkv_weights, split_size, dim=0)

            self.q_weight_ = q_weights[split_n_embed *
                                       self.tp_rank_: split_n_embed *
                                       (self.tp_rank_ +
                                        1), :]
            self.q_weight_ = self._cuda(self.q_weight_.transpose(0, 1))
            self.k_weight_ = k_weights[split_n_embed *
                                       self.tp_rank_: split_n_embed *
                                       (self.tp_rank_ +
                                        1), :]
            self.k_weight_ = self._cuda(self.k_weight_.transpose(0, 1))
            self.v_weight_ = v_weights[split_n_embed *
                                       self.tp_rank_: split_n_embed *
                                       (self.tp_rank_ +
                                        1), :]
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_attn.bias" in weights:
            ...
            self.q_bias_ = self._cuda(
                q_bias[split_n_embed * self.tp_rank_: split_n_embed * (self.tp_rank_ + 1)])
            self.k_bias_ = self._cuda(
                k_bias[split_n_embed * self.tp_rank_: split_n_embed * (self.tp_rank_ + 1)])
            self.v_bias_ = self._cuda(
                v_bias[split_n_embed * self.tp_rank_: split_n_embed * (self.tp_rank_ + 1)])

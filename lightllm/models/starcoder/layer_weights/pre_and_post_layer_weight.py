...
class StarcoderPreAndPostLayerWeight(PreAndPostLayerWeight):
    ...
    def load_hf_weights(self, weights):
        ...
        if "transformer.wpe.weight" in weights:
            ...
            self.wpe_weight_ = weights['transformer.wpe.weight'].to(
        ...
        if "transformer.ln_f.weight" in weights:
            self.final_norm_weight_ = weights['transformer.ln_f.weight'].contiguous().to(
                self.data_type_).cuda()
        if "transformer.ln_f.bias" in weights:
            self.final_norm_bias_ = weights["transformer.ln_f.bias"].contiguous().to(
                self.data_type_).cuda()
        return

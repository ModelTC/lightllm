    ...
    def _load_qkvo_weights(self, weights):
        ...
        if f"transformer.h.{self.layer_num_}.ln_1.weight" in weights:
            self.att_norm_weight_ = (
                weights[f"transformer.h.{self.layer_num_}.ln_1.weight"]
                .to(self.data_type_)
                .cuda()
            )
        ...
        if f"transformer.h.{self.layer_num_}.ln_1.bias" in weights:
            self.att_norm_bias_ = (
                weights[f"transformer.h.{self.layer_num_}.ln_1.bias"]
                .to(self.data_type_)
                .cuda()
            )
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_attn.weight" in weights:
            ...
            self.q_weight_ = self.qkv_weight_[:, :n_embed][
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_attn.bias" in weights:
            self.qkv_bias_ = weights[
                f"transformer.h.{self.layer_num_}.attn.c_attn.bias"
            ].to(self.data_type_)
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_proj.weight" in weights:
            self.o_weight_ = weights[
                f"transformer.h.{self.layer_num_}.attn.c_proj.weight"
            ][:, split_n_embed * self.tp_rank_ : split_n_embed * (self.tp_rank_ + 1)]
            self.o_weight_ = (
                self.o_weight_.transpose(0, 1).contiguous().to(self.data_type_)
            )
            self.o_weight_ = self.o_weight_.cuda()

        if f"transformer.h.{self.layer_num_}.attn.c_proj.bias" in weights:
            self.o_bias_ = weights[
                f"transformer.h.{self.layer_num_}.attn.c_proj.bias"
            ].to(self.data_type_)
    ...
    def _load_ffn_weights(self, weights):
        if f"transformer.h.{self.layer_num_}.ln_2.weight" in weights:
            self.ffn_norm_weight_ = (
                weights[f"transformer.h.{self.layer_num_}.ln_2.weight"]
                .to(self.data_type_)
                .cuda()
            )
        if f"transformer.h.{self.layer_num_}.ln_2.bias" in weights:
            self.ffn_norm_bias_ = (
                weights[f"transformer.h.{self.layer_num_}.ln_2.bias"]
                .to(self.data_type_)
                .cuda()
            )
        ...
        if f"transformer.h.{self.layer_num_}.mlp.c_fc.weight" in weights:
            self.ffn_1_weight_ = weights[
                f"transformer.h.{self.layer_num_}.mlp.c_fc.weight"
            ].to(self.data_type_)
            self.ffn_1_weight_ = (
                self.ffn_1_weight_[
                    split_inter_size
                    * self.tp_rank_ : split_inter_size
                    * (self.tp_rank_ + 1),
                    :,
                ]
                .transpose(0, 1)
        ...
        if f"transformer.h.{self.layer_num_}.mlp.c_proj.weight" in weights:
            self.ffn_2_weight_ = weights[
                f"transformer.h.{self.layer_num_}.mlp.c_proj.weight"
            ].to(self.data_type_)
            self.ffn_2_weight_ = (
                self.ffn_2_weight_[
                    :,
        ...
        if f"transformer.h.{self.layer_num_}.mlp.c_proj.bias" in weights:
            self.ffn_2_bias_ = (
                weights[f"transformer.h.{self.layer_num_}.mlp.c_proj.bias"]
                .to(self.data_type_)
                .contiguous()
                .cuda()
            )

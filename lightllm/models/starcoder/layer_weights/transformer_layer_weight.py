...
class StarcoderTransformerLayerWeight(BloomTransformerLayerWeight):
    ...
    def _load_qkvo_weights(self, weights):
        ...
        if f"transformer.h.{self.layer_num_}.ln_1.weight" in weights:
            self.att_norm_weight_ = weights[f"transformer.h.{self.layer_num_}.ln_1.weight"].to(
                self.data_type_).cuda()

        if f"transformer.h.{self.layer_num_}.ln_1.bias" in weights:
            self.att_norm_bias_ = weights[f"transformer.h.{self.layer_num_}.ln_1.bias"].to(
                self.data_type_).cuda()

        # attention params
        n_embed = self.network_config_["hidden_size"]
        head_dim = self.network_config_[
            "hidden_size"] // self.network_config_["num_attention_heads"]
        split_n_embed = n_embed // self.world_size_
        if f"transformer.h.{self.layer_num_}.attn.c_attn.weight" in weights:
            self.qkv_weight_ = weights[f"transformer.h.{self.layer_num_}.attn.c_attn.weight"].transpose(
                0, 1).contiguous().to(self.data_type_)
            self.q_weight_ = self.qkv_weight_[
            ...
            self.v_weight_ = self.qkv_weight_[
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_attn.bias" in weights:
            ...
            self.qkv_bias_ = weights[f"transformer.h.{self.layer_num_}.attn.c_attn.bias"].to(
                self.data_type_)
            self.q_bias_ = self.qkv_bias_[:n_embed].cuda(
            )[split_n_embed * self.tp_rank_: split_n_embed * (self.tp_rank_ + 1)]
            self.k_bias_ = self.qkv_bias_[n_embed: n_embed + head_dim].cuda()
            self.v_bias_ = self.qkv_bias_[
                n_embed + head_dim: n_embed + 2 * head_dim].cuda()
        ...
        if f"transformer.h.{self.layer_num_}.attn.c_proj.weight" in weights:
            self.o_weight_ = weights[f"transformer.h.{self.layer_num_}.attn.c_proj.weight"][:,
                                                                                            split_n_embed * self.tp_rank_: split_n_embed * (self.tp_rank_ + 1)]
            self.o_weight_ = self.o_weight_.transpose(
                0, 1).contiguous().to(self.data_type_)
            self.o_weight_ = self.o_weight_.cuda()

        if f"transformer.h.{self.layer_num_}.attn.c_proj.bias" in weights:
            self.o_bias_ = weights[f"transformer.h.{self.layer_num_}.attn.c_proj.bias"].to(
                self.data_type_)
    ...
    def _load_ffn_weights(self, weights):
        ...
        if f"transformer.h.{self.layer_num_}.mlp.c_fc.weight" in weights:
            self.ffn_1_weight_ = weights[f"transformer.h.{self.layer_num_}.mlp.c_fc.weight"].to(
                self.data_type_)
        ...
        if f"transformer.h.{self.layer_num_}.mlp.c_proj.weight" in weights:
            self.ffn_2_weight_ = weights[f"transformer.h.{self.layer_num_}.mlp.c_proj.weight"].to(
                self.data_type_)
        ...
        if f"transformer.h.{self.layer_num_}.mlp.c_proj.bias" in weights:
            self.ffn_2_bias_ = weights[f"transformer.h.{self.layer_num_}.mlp.c_proj.bias"].to(
                self.data_type_).contiguous().cuda()

...
class ViTPostLayerInfer:
    ...
    def forward(self, vit_embeds, layer_weight: ViTPreAndPostLayerWeight):
        ...
        vit_embeds_norm = torch.nn.functional.layer_norm(vit_embeds, (vit_embeds.shape[-1],), 
                          weight=layer_weight.layernorm_weight_, bias=layer_weight.layernorm_bias_)

        vit_embeds_1 = torch.addmm(layer_weight.mlp1_1_bias_, vit_embeds_norm.view(-1, vit_embeds_norm.shape[-1]), 
                                   layer_weight.mlp1_1_weight_)
        
        vit_embeds_gelu = torch.nn.functional.gelu(vit_embeds_1)

        vit_embeds_out = torch.addmm(layer_weight.mlp1_3_bias_, vit_embeds_gelu.view(-1, self.llm_hidden_size // self.world_size_), 
                                   layer_weight.mlp1_3_weight_, beta = 1. / self.world_size_)

...
class ViTPreLayerInfer(PreLayerInferTpl):
    ...
    def forward(self, pixel_values, layer_weight: ViTPreAndPostLayerWeight):
        ...
        patch_embeds = F.conv2d(pixel_values, weight= layer_weight.patch_embedding_weight_,
                                bias= layer_weight.patch_embedding_bias_, stride=layer_weight.patch_size)
        ...
        if self.world_size_ == 1:
            return embeddings
        gather_embedding = torch.empty((embeddings.shape[2] * self.world_size_, batch_size, embeddings.shape[1]), 
                                   device=embeddings.device, dtype=target_dtype)

        ...
        for stop_token_ids in self.sample_params.stop_sequences:
            ...
            if stop_len > 0:
                if len(self.output_ids) >= stop_len:
                    if all(
                        self.output_ids[-(stop_len - i)] == stop_token_ids[i]
...
class NormalReq(Req):
    def __init__(
        ...
        super().__init__(
            request_id, prompt_ids, sample_params, prompt_cache_len, prompt_cache_req_id
        )
        return
    ...
    def get_tuple_tokens(self, is_busy, router_max_new_token_len):
        ...
        if self.req_status == ReqRunStatus.RUNNING:
            return (
                self.input_len + has_out_len - self.prompt_cache_len,
                max(0, cur_max_new_token_len - has_out_len - 1),
            )
        elif self.req_status == ReqRunStatus.WAIT_IN_QUEUE:
            return (
                self.input_len + 1 - self.prompt_cache_len,
                max(0, cur_max_new_token_len - 1 - 1),
            )
        elif self.req_status == ReqRunStatus.PAUSED_AND_OFFLOAD:
            return (
                self.input_len + has_out_len + 1 - self.prompt_cache_len,
                max(0, cur_max_new_token_len - has_out_len - 1 - 1),
            )
...
class SplitFuseReq(Req):
    ...
    def get_tuple_tokens(self, is_busy, router_max_new_token_len):
        ...
        if (
            self.req_status == ReqRunStatus.RUNNING
            or self.req_status == ReqRunStatus.PAUSED_AND_KVKEEP
        ):
            return (
                self.input_len + has_out_len - self.prompt_cache_len,
                max(
            ...
            return (
                self.input_len - self.prompt_cache_len,
                max(
                    0,
                    (
                        self.input_len
    ...
    def get_decode_need_tokens(self):
        ...
        if self.req_status == ReqRunStatus.RUNNING:
            return min(
                self.input_len + len(self.output_ids) - self.cur_kv_len,
                self.splitfuse_block_size,
            )
        else:
            assert False, "error state"
    ...
    def get_first_router_need_tokens(self):
        if self.req_status == ReqRunStatus.WAIT_IN_QUEUE:
            return min(
                self.input_len - self.prompt_cache_len, self.splitfuse_block_size
            )
        elif self.req_status == ReqRunStatus.PAUSED_AND_OFFLOAD:
            return min(
                self.input_len + len(self.output_ids) - self.prompt_cache_len,
                self.splitfuse_block_size,
            )
        elif self.req_status == ReqRunStatus.PAUSED_AND_KVKEEP:
            return min(
    ...
    def mark_and_get_finished_req_and_preupdate_status(self, eos_id):
        ...
        for req in self.reqs:
            ...
            if (
                ...
                and req.output_ids[-1] == eos_id
                and req.sample_params.ignore_eos == False
            ):
                req.has_generate_finished = True

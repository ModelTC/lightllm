...
class TreeNode:
    ...
    def split_node(self, prefix_len):
        ...
        split_parent_node.shared_idx_node.set_node_prefix_total_len(split_parent_node.get_parent_prefix_total_len() + new_len)
...
class RadixCache:
    def __init__(self, total_token_num, tp_id):
        ...
        self.shared_idx_manager = SharedLinkedListManager(total_token_num, tp_id)
...
if __name__ == "__main__":
    ...
    def test2():
        ...
        tree_node, size, values = tree.match_prefix(torch.tensor([0, 1, 2, 3, 4], dtype=torch.int64, device="cpu"), update_refs=False)
        assert tree_node.shared_idx_node.get_node_prefix_total_len() == 5 and size == 5 and len(values) == 5
        tree_node, size, values = tree.match_prefix(torch.tensor([0, 1, 2, 3, 4, 9], dtype=torch.int64, device="cpu"), update_refs=False)
        assert tree_node.shared_idx_node.get_node_prefix_total_len() == 5 and size == 5 and len(values) == 5
        tree_node, size, values = tree.match_prefix(torch.tensor([0, 1, 2, 3, 4, 7, 8], dtype=torch.int64, device="cpu"), update_refs=False)
        assert tree_node.shared_idx_node.get_node_prefix_total_len() == 7 and size == 7 and len(values) == 7
        tree_node, size, values = tree.match_prefix(torch.tensor([0, 1, 2, 3, 4, 7, 9], dtype=torch.int64, device="cpu"), update_refs=False)
    ...
    def test3():
        ...
        tree_node, size, values = tree.match_prefix(torch.tensor([0, 1, 2, 3, 4, 7, 9], dtype=torch.int64, device="cpu"), update_refs=True)
        assert tree_node.shared_idx_node.get_node_prefix_total_len() == 6 and size == 6 and len(values) == 6

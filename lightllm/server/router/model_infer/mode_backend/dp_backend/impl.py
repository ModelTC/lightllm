...
class DPBackend(ModeBackend):
    ...
    def prefill(self, reqs: List[Tuple]):
        ...
        kwargs, run_reqs, padding_token_num = prepare_prefill_inputs(req_ids, self.is_multimodal)
    ...
    def decode(self):
        kwargs, run_reqs, padding_token_num = prepare_decode_inputs(g_infer_context.infer_req_ids)
        logits = self.model.forward(**kwargs)
        if len(run_reqs) == 0:
            return
        else:

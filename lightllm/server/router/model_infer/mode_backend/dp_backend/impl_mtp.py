...
class DPChunkedPrefillWithMTPBackend(ContinuesBatchWithMTPBackend):
    ...
    def init_custom(self):
        ...
        model_input, run_reqs = prepare_prefill_inputs(
            [], is_chuncked_mode=True, is_multimodal=self.is_multimodal, pad_for_empty_batch=True
        )
    ...
    def normal_prefill_reqs(self, prefill_reqs: List[InferReq], max_prefill_num: int, uninit_reqs, ok_finished_reqs):
        ...
        for draft_model_idx in range(self.spec_step):
            ...
            draft_model_input, last_input_ids_cpu, prev_step_has_output = prepare_mtp_prefill_inputs(
                prefill_reqs,
                model_input,
                last_hidden_states,
        ...
        if all_run_reqs:
            ...
            for draft_model_idx in range(self.spec_step):
                ...
                if all_draft_logits is None:
                    all_draft_logits = torch.empty(
                        (req_num + req_num1, draft_micro_output.logits.shape[1]),
                        dtype=draft_micro_output.logits.dtype,
                        device=draft_micro_output.logits.device,
                    )

                all_draft_logits[0:req_num, :].copy_(draft_micro_output.logits[0:req_num, :], non_blocking=True)
                all_draft_logits[req_num : (req_num + req_num1), :].copy_(
                    draft_micro_output1.logits[0:req_num1, :], non_blocking=True
                )
    ...
    def overlap_prefill_reqs(self, prefill_reqs: List[InferReq], max_prefill_num: int, uninit_reqs, ok_finished_reqs):
        ...
        if all_run_reqs:
            ...
            for draft_model_idx in range(self.spec_step):
                ...
                draft_micro_input0, last_input_ids_cpu0, prev_step_has_output0 = prepare_mtp_prefill_inputs(
                    run_reqs0,
                    draft_micro_input0,
                    last_hidden_states0,
                ...
                draft_micro_input1, last_input_ids_cpu1, prev_step_has_output1 = prepare_mtp_prefill_inputs(
                    run_reqs1,
                    draft_micro_input1,
                    last_hidden_states1,
                ...
                if all_draft_logits is None:
                    all_draft_logits = torch.empty(
                        (req_num0 + req_num1, draft_micro_output0.logits.shape[1]),
                        dtype=draft_micro_output0.logits.dtype,
                        device=draft_micro_output0.logits.device,
                    )

                all_draft_logits[0:req_num0, :].copy_(draft_micro_output0.logits[0:req_num0, :], non_blocking=True)
                all_draft_logits[req_num0 : (req_num0 + req_num1), :].copy_(
                    draft_micro_output1.logits[0:req_num1, :], non_blocking=True
                )

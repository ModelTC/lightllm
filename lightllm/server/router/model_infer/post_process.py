...
def sample(logits, batch: InferBatch):
    ...
    sampled_index = torch.multinomial(
        probs_sort, num_samples=1, replacement=True)
...
def _top_p_top_k(probs: torch.Tensor, top_ps: torch.Tensor,
                 top_ks: torch.Tensor):
    ...
    probs_sort[torch.arange(
        0, probs.shape[-1], device="cuda").view(1, -1) >= top_ks.view(-1, 1)] = 0.0

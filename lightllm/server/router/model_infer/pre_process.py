...
def prepare_prefill_inputs(batch: InferBatch):
    ...
    if len(run_reqs) >= 1:
        ...
        nopad_b_req_idx = torch.tensor(
            nopad_b_req_idx, dtype=torch.int32, device='cuda'
        )
        nopad_b_start_loc = torch.tensor(
            nopad_b_start_loc, dtype=torch.int32, device='cuda'
        )
        nopad_b_seq_len = torch.tensor(
            nopad_b_seq_len, dtype=torch.int32, device='cuda'
        )
        kwargs = {
...
def splitfuse_prepare_decode_inputs(batch: InferBatch, splitfuse_block_size):
    ...
    for req in prefill_reqs:
        ...
        prefill_max_split_seq_len_in_batch = max(
            prefill_max_split_seq_len_in_batch, split_len
        )
    ...
    kwargs = {
        ...
        "decode_b_req_idx": torch.tensor(
            decode_b_req_idx, dtype=torch.int32, device='cuda'
        ),
        "decode_b_start_loc": torch.tensor(
            decode_b_start_loc, dtype=torch.int32, device="cuda"
        ),
        "decode_b_seq_len": torch.tensor(
            decode_b_seq_len, dtype=torch.int32, device="cuda"
        ),
        "decode_max_len_in_batch": decode_max_len_in_batch,
        "prefill_req_num": prefill_req_num,
        "prefill_b_req_idx": torch.tensor(
            prefill_b_req_idx, dtype=torch.int32, device="cuda"
        ),
        "prefill_b_split_start_loc": torch.tensor(
            prefill_b_split_start_loc, dtype=torch.int32, device="cuda"
        ),
        "prefill_b_split_seq_len": torch.tensor(
            prefill_b_split_seq_len, dtype=torch.int32, device="cuda"
        ),
        "prefill_max_split_seq_len_in_batch": prefill_max_split_seq_len_in_batch,
        "prefill_b_seq_len": torch.tensor(
            prefill_b_seq_len, dtype=torch.int32, device="cuda"
        ),
    }

...
class Sfj(Strategy):
    ...
    def ordering_reqs(self, batch: Batch):
        reqs = [req for req in batch.reqs]
        return sorted(
            reqs, key=lambda req: req.max_output_len - len(req.output_ids), reverse=True
        )
...
class Hrnn(Strategy):
    ...
    def ordering_reqs(self, batch: Batch):
        ...
        return sorted(
            reqs,
            key=lambda req: (req.input_len + req.max_output_len - len(req.output_ids))
            / req.input_len,
            reverse=True,
        )
...
def select_paused_reqs(
    batch: Batch, strategy: Strategy, req_queue: ReqQueue, max_total_token_num
):

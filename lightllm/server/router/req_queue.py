...
class ReqQueue:
    ...
    def __init__(self, max_total_tokens, allow_finish_percent, batch_max_tokens, running_max_req_size, ema, token_ratio) -> None:
    ...
    def _init_cache_list(self, current_batch:Batch, token_ratio=0.):
        if current_batch is not None:
            self.cache_len_list = [(req.input_len + len(req.output_ids) - 1, max(1, self.ema.get_max_output_len(req, token_ratio) - len(req.output_ids))) for req in current_batch.reqs]
        else:
            self.cache_len_list = []

    def _can_add_new_req(self, req, token_ratio=0.):
        self.cache_len_list.append((req.calcu_need_tokens(), self.ema.get_max_output_len(req, token_ratio) - len(req.output_ids) - 1)) # hard to analysis

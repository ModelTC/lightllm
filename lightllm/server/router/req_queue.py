...
class ReqQueue:
    def __init__(self, args, prompt_cache_used_tokens, prompt_cache_req_num) -> None:
        ...
        self.pause_req_dict = (
            {}
        )  # 用于保存队列中被暂停的请求，暂停原因为 ReqRunStatus.PAUSED_AND_KVKEEP  ReqRunStatus.PAUSED_AND_OFFLOAD
    ...
    def _can_add_new_req(self, req: Req, is_busy):
        ...
        ok_req_num = (
            len(self.cache_len_list)
            + self.cache_pause_reqs_num
    ...
    def generate_new_batch(self, current_batch: Batch):
        ...
        cur_all_used_tokens = (
            0 if current_batch is None else current_batch.batch_used_tokens
        )
        cur_all_used_tokens += (
            self.recalcu_pause_req_used_tokens() + self.prompt_cache_used_tokens
        )
        ...
        if not self.is_splitfuse_mode:
            ...
            cur_batch_decode_need_tokens = (
                0 if current_batch is None else current_batch.batch_decode_need_tokens
            )
        ...
        for req in self.waiting_req_list:
            ...
            if (
                ...
                and cur_batch_decode_need_tokens
                + new_batch_first_router_need_tokens
                + req_first_router_need_tokens

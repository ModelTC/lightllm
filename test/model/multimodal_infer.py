...
def test_multimodal_inference(world_size, model_dir, model_class, batch_size, input_len, output_len, mode, multimodal_kwargs):
    ans_queue = Queue()
    workers = []
    input_len += multimodal_kwargs.pop("pad_len")
    for rank_id in range(world_size):
        model_kvargs = {
            ...
            "multimodal_input": multimodal_kwargs,
        }
        
        proc = multiprocessing.Process(target=tppart_multimodal_infer, args=(model_class, model_kvargs, batch_size, input_len, output_len, ans_queue))
        proc.start()
        workers.append(proc)
...
def tppart_multimodal_infer(model_class, model_kvargs, batch_size, input_len, output_len, ans_queue):
    import torch
    ...
    multimodal_inputs = [model_kvargs["multimodal_input"] for _ in range(batch_size)]
    ...
    logics = model_part.forward(batch_size, total_token_num, input_len, test_data,
                                                 b_req_idx, b_start_loc, b_seq_len, is_prefill=True, multimodal_inputs=multimodal_inputs)
    prob_out = torch.softmax(logics, dim=-1)
    predict_ids = torch.argmax(prob_out, dim=1, keepdim=True)
    predict_ids = predict_ids.detach().cpu().numpy()

...
def tppart_model_infer(
    model_class, model_kvargs, batch_sizes, input_len, output_len, ans_queue, log_path
):
    ...
    for batch_size in batch_sizes:
        ...
        if os.path.exists(new_log_path):
            with open(new_log_path, "r") as fp_file:
                ...
                if (
                    len(lines) >= 2 and "time total cost(ms):" in lines[-1]
                ):  # 说明已经跑过了，直接过滤掉。
                    continue
                else:
    ...
    dist.init_process_group(
        'nccl', init_method='tcp://127.0.0.1:28765', rank=rank_id, world_size=world_size
    )
    ...
    for batch_size in need_run_batch_sizes:
        ...
        for i in range(output_len):
            b_start_loc = b_start_loc + torch.arange(
                0, batch_size, dtype=torch.int32, device="cuda"
            )
        ...
        if rank_id == 0:
            print(
                "prefill time cost:",
                (time.time() - prefill_start_time) * 1000,
                file=fp_file,
            )

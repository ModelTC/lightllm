...
def test_all_setting(
    gpu_name, model_name, mode, log_dir, world_sizes, in_out_lens, batch_sizes
):
    ...
    for log_file in log_files:
        ...
        card_num_per_qps = float(world_size) / (
            float(batch_size) / (all_step_cost / 1000)
        )
        md_file.write('|')
        infos = [
            world_size,
        ...
        for info in infos:
            md_file.write(
                str(format(info, ".4f")) if isinstance(info, float) else str(info)
            )
            md_file.write("|")
        md_file.write('\r\n')
    md_file.close()

...
class StarcoderTransformerLayerInferINT8(StarcoderTransformerLayerInfer):
    ...
    def _get_qkv(self, input, infer_state: StarcoderInferStateInfo, layer_weight: StarcoderTransformerLayerWeightQuantized,
        ...
        # print("_get_qkv", layer_weight.qkv_fused_bias.shape, input.shape, layer_weight.qkv_fused_weight.shape, layer_weight.qkv_fused_weight_scale.shape, qkv_output.shape, self.head_dim, q.shape, k.shape, v.shape, cache_k.shape, cache_v.shape, self.tp_k_head_num_, self.head_dim_)
    ...
    def _get_o_decode(self, input, infer_state: StarcoderInferStateInfo, layer_weight: StarcoderTransformerLayerWeightQuantized) -> torch.Tensor:
        o_tensor = layer_weight.o_bias_ + matmul_quantize_int8(input.view(-1, self.tp_o_head_num_ * self.head_dim_), layer_weight.o_weight_, layer_weight.o_weight_scale_)
        return o_tensor
